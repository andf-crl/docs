...
toc: true
affected_[version](cluster-settings.html#setting-version)s: v21.1 and v21.2 prior to v21.1.13 and v21.2.3
...
In [version](cluster-settings.html#setting-version)s of CockroachDB 21.1 and 21.2 prior to 21.1.13 and 21.2.3, planning queries over partitioned tables with a `DEFAULT` partition in a `PARTITION BY LIST` clause could cause a spurious internal error with text “empty Datums being compared to other”. This issue could occur when a query contained a `WHERE` clause including at least one indexed column from the partitioned index, but not the first column.

The issue does not exist on 20.2 and prior [version](cluster-settings.html#setting-version)s.
...
Users of CockroachDB 21.1 and 21.2 using partitioned tables with a `DEFAULT` partition in a `PARTITION BY LIST` clause are encouraged to upgrade to 21.1.13, 21.2.3, or a later [version](cluster-settings.html#setting-version).  To avoid further errors while still on prior [version](cluster-settings.html#setting-version)s, users can either remove the `DEFAULT` partition, or if this is not possible, ensure that their queries always have a predicate on the first column of the partitioned index when there is a `WHERE` clause. For example:
