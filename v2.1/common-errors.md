...
To resolve this issue, install the `tzdata` library and keep it up-to-date. It's important for all nodes to have the same [version](cluster-settings.html#setting-version), so when updating the library, do so as quickly as possible across all nodes.
