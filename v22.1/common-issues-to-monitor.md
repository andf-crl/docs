...
-  The "active query dump", enabled by default with the `[diagnostics.active_query_dumps.enabled](cluster-settings.html#setting-diagnostics-active_query_dumps-enabled)` [cluster setting](cluster-settings.html), is a record of anonymized active queries that is written to disk when a node is detected to be under memory pressure.
