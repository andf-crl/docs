...
- Access to a CockroachDB cluster as [`admin` SQL user](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/security-reference/authorization.html#admin-role). This tutorial will use a {{ site.data.products.serverless }} cluster, but you may either [Create a {{ site.data.products.serverless }} cluster](../cockroachcloud/create-a-serverless-cluster.html) or [Start a Local Cluster (secure)](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/start-a-local-cluster.html) in order to follow along. In either case you must have the public CA certificate for your cluster, and a username/password combination for the `root` SQL user (or another SQL user with the [`admin` role](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/security-reference/authorization.html#admin-role).
