...
| Identifier length | 128 bytes | This limit is specified in the `max_identifier_length` variable for compatibility with other databases, but is not enforced. It may be enforced in future [version](cluster-settings.html#setting-version)s of CockroachDB, so we recommended remaining within this limit. |
