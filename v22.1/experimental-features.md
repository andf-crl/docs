...
{% include_cached new-in.html [version](cluster-settings.html#setting-version)="v22.1" %} If you prefer to use an approach to relocating replicas and leases based on range IDs, see the [`ALTER RANGE ... RELOCATE`](alter-range-relocate.html) statement.
{{site.data.alerts.end}}
