...
KV 95 is a simple benchmark that tests linear scaling by [running a workload](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/cockroach-workload.html#workloads) that is 95% point reads and 5% point writes. Reads and writes are distributed to keys spread uniformly across the cluster.
...
- See [CockroachDB Performance](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/performance.html) for more information about CockroachDB performance benchmarking.
- See [SQL Performance Best Practices](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/performance-best-practices-overview.html) for guidance on tuning real workloads.
