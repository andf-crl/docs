...
5. Select both networks: **DB Console to monitor the cluster** and **CockroachDB Client to access the databases**.
    ...
    The **DB Console** refers to the cluster's DB Console, where you can observe your cluster's health and performance. For more information, see [DB Console Overview](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/ui-overview.html).
...
1. If you haven't already, [Download the CockroachDB binary](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/install-cockroachdb.html) and copy it into the `PATH`:
...
On the [**Cluster Overview** page](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/ui-cluster-overview-page.html), view essential metrics about the cluster's health:
...
2. On the [**Hardware** dashboard](../{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/ui-hardware-dashboard.html), view metrics about CPU usage, disk throughput, network traffic, storage capacity, and memory.
