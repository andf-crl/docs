...
<span class="[version](cluster-settings.html#setting-version)-tag">New in v1.1:</span> The `DROP USER` [statement](sql-statements.html) removes one or more SQL users.
...
<span class="[version](cluster-settings.html#setting-version)-tag">New in v2.0:</span> All of a user's privileges must be revoked before the user can be dropped.
