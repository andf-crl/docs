...
- A CockroachDB node started with [`cockroach start`](../v20.2/cockroach-start.html) without the `--join` flag no longer automatically initializes the cluster. The `cockroach init` command is now mandatory. The auto-initialization behavior had been deprecated in [version](cluster-settings.html#setting-version) 19.2. [#51245][#51245]
...
- Reverted the Go [version](cluster-settings.html#setting-version) back to 1.13. [#52348][#52348]
...
- The `[sql.log.slow_query.latency_threshold](cluster-settings.html#setting-sql-log-slow_query-latency_threshold)` cluster setting now requires an explicit unit when being set. (e.g., 500ms for  500 milliseconds, 5us for 5 nanoseconds, 5s for 5 seconds, etc.). An error with a hint pops up if the user fails to provide units. [#50632][#50632]
...
- Added the cluster setting `[sql.defaults.idle_in_session_timeout](cluster-settings.html#setting-sql-defaults-idle_in_session_timeout)` for setting a default `idle_in_session_timeout` value for new sessions. This gives users the ability to set `idle_in_session_timeout` values for all newly created sessions. [#51223][#51223]
...
- The [`EXPORT`](../v20.2/export.html) statement no longer requires an [enterprise.license](cluster-settings.html#setting-enterprise-license). [#52029][#52029]
...
- The v20.2 CLI [`cockroach node`](../v20.2/cockroach-node.html) family of subcommands will not work with servers running older [version](cluster-settings.html#setting-version)s of `cockroach`, but the v20.1 CLI `cockroach node` subcommands will work against v20.2 servers. [#50329][#50329]
...
- Updated the label used for the commit ID in the printed [version](cluster-settings.html#setting-version) info. [#52263][#52263]
...
- Fixed a bug causing the raw trace file collected inside a statement [diagnostics bundle](../v20.2/explain-analyze.html#debug-option) to be sometimes empty when the cluster setting `[sql.trace.txn.enable_threshold](cluster-settings.html#setting-sql-trace-txn-enable_threshold)` was in use. [#50914][#50914]
...
- Updated the functions `crdb_[version](cluster-settings.html#setting-version)()` and `[version](cluster-settings.html#setting-version)()` to return the same result. [#52032][#52032]
...
- Fixed a bug where jobs could fail to run during [version](cluster-settings.html#setting-version) upgrades. [#52475][#52475]
...
- Improved the efficiency of garbage collection when there are a large number of [version](cluster-settings.html#setting-version)s of a single key, commonly found when utilizing sequences. [#51184][#51184]
