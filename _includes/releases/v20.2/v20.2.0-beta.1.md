...
- Certificate revocation is now supported via OSCP and the [cluster setting](../v20.2/cluster-settings.html) `[security.ocsp.mode](cluster-settings.html#setting-security-ocsp-mode)`. This makes a call to the OCSP server on every connection attempt and may wait for up to value specified with the setting `[security.ocsp.timeout](cluster-settings.html#setting-security-ocsp-timeout)`. [#53218][#53218] {% comment %}doc{% endcomment %}
...
- Roles that had the `CREATEROLE` privilege prior to upgrading to this [version](cluster-settings.html#setting-version) are also automatically granted `CREATELOGIN`. After the upgrade, `CREATELOGIN` is no longer granted automatically. [#50601][#50601] {% comment %}doc{% endcomment %}
...
- As part of making basic [`BACKUP`](../v20.2/backup.html) and [`RESTORE`](../v20.2/restore.html) free to use without an [enterprise.license](cluster-settings.html#setting-enterprise-license), [`SHOW BACKUP`](../v20.2/show-backup.html) and `SHOW BACKUPS IN` no longer require a license. [#53356][#53356] {% comment %}doc{% endcomment %}
...
- Box2d comparison operators are now gated by the [cluster setting](../v20.2/cluster-settings.html) `[sql.spatial.experimental_box2d_comparison_operators.enabled](cluster-settings.html#setting-sql-spatial-experimental_box2d_comparison_operators-enabled)`. [#52990][#52990] {% comment %}doc{% endcomment %}
...
- Added a new public [cluster setting](../v20.2/cluster-settings.html) `[sql.defaults.disallow_full_table_scans.enabled](cluster-settings.html#setting-sql-defaults-disallow_full_table_scans-enabled)` that defaults to false, which informs the session setting `disallow_full_table_scans`. If the session is configured to disallow full table/index scans, any user query that plans a full table/index scan is rejected. This does not apply to internal queries or queries over virtual schemas (e.g., `pg_catalog`). [#52278][#52278] {% comment %}doc{% endcomment %}
...
- The command-line flag `--insecure` has been marked as deprecated. See [issue #53404](https://go.crdb.dev/issue/53404) for details. The flag will be removed in a later [version](cluster-settings.html#setting-version) in a staged fashion: first, additional security mechanisms will be added to enable more flexible deployments which were previously done using `--insecure`; then the flag will be removed from server commands; then finally, in a later [version](cluster-settings.html#setting-version), also from client commands. [#53405][#53405] {% comment %}doc{% endcomment %}
...
- The `--geo-libs` flag introduced in v20.2 alpha [version](cluster-settings.html#setting-version)s is now renamed to `--spatial-libs`. [#53721][#53721] {% comment %}doc{% endcomment %}
- The CLI no longer prints a blanket `Time` for queries. Instead, if `show_times` is turned on and the server [version](cluster-settings.html#setting-version) is v20.2 or later, the CLI prints two separate times: the server execution time and the network latency. [#52233][#52233] {% comment %}doc{% endcomment %}
...
- Fixed a bug when running restore while [upgrading the cluster from v20.1 to v20.2](../v20.2/upgrade-cockroach-[version](cluster-settings.html#setting-version).html). [#53848][#53848]
- Fixed a bug where we allowed new types to be used in an array type during a [version](cluster-settings.html#setting-version) upgrade. [#53961][#53961]
