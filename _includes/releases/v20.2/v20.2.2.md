...
- [`ALTER TABLE ... SET DATA TYPE ...`](../v20.2/alter-column.html) is no longer available for operations that involve con[version](cluster-settings.html#setting-version) (e.g., [`STRING`](../v20.2/string.html) to [`TIMESTAMPTZ`](../v20.2/timestamp.html)) or precision/width truncation (e.g., [`INT(4)`](../v20.2/int.html) -> `INT(2)`) due to a bug in validation. These features are already gated by the [session variable](../v20.2/set-vars.html) `enable_experimental_alter_column_type_general` - setting this session variable is now a no-op. [#56629][#56629]
- Previously, `timestamp/timestamptz - timestamp/timestamptz` operators would normalize the interval into months, days, `H:M:S` (in older [version](cluster-settings.html#setting-version)s, this may be just `H:M:S`). This can give an incorrect result:
...
- Previously, the `age` built-in would incorrectly normalize months and days based on 30 days a month (in older [version](cluster-settings.html#setting-version)s this may be just `H:M:S`). This can give an incorrect result:
...
- Added a new [cluster setting](../v20.2/cluster-settings.html) called `[server.oidc_authentication.autologin](cluster-settings.html#setting-server-oidc_authentication-autologin)` which enables an automatic redirect to the OIDC login flow instead of showing a password login prompt. A query parameter can force disable this feature in the browser by appending `?oidc_auto_login=false` to the login path. [#56510][#56510]
- Adjusted the styles for the [Session Details page](../v20.2/ui-sessions-page.html). [#55889][#55889]
- [Transaction statistics](../v20.2/ui-sql-dashboard.html#transactions) are no longer recorded if the `[sql.metrics.statement_details.threshold](cluster-settings.html#setting-sql-metrics-statement_details-threshold)` [cluster setting](../v20.2/cluster-settings.html) has been enabled. [#56380][#56380]
