...
- Prior to this patch, the phase of server shutdown responsible for range lease transfers to other nodes would give up after 10000 attempts of transferring replica leases away, regardless of the value of `[server.shutdown.lease_transfer_wait](cluster-settings.html#setting-server-shutdown-lease_transfer_wait)`. The limit of 10000 attempts has been removed, so that now only the maximum duration `[server.shutdown.lease_transfer_wait](cluster-settings.html#setting-server-shutdown-lease_transfer_wait)` applies. [#45149][#45149]

<h3 id="v20-2-0-alpha-1-general-changes">General changes</h3>

- Transactions reading a lot of data behave better when exceeding the memory limit set by `[kv.transaction.max_refresh_spans_bytes](cluster-settings.html#setting-kv-transaction-max_refresh_spans_bytes)`. Such transactions now attempt to resolve the conflicts they run into instead of being forced to always retry. Increasing `[kv.transaction.max_refresh_spans_bytes](cluster-settings.html#setting-kv-transaction-max_refresh_spans_bytes)` should no longer be necessary for most workloads. [#46275][#46275]
...
- Prior to this patch, the phase of server shutdown responsible for range lease transfers to other nodes had a hard timeout of 5 seconds. This patch makes this timeout configurable via the new [cluster setting](../v20.2/cluster-settings.html) `[server.shutdown.lease_transfer_wait](cluster-settings.html#setting-server-shutdown-lease_transfer_wait)`. [#45149][#45149]
...
- Improved the error message for [`ALTER COLUMN ... SET DATA TYPE ...`](../v20.2/alter-column.html) for data type con[version](cluster-settings.html#setting-version)s that involve overwriting existing values. [#47155][#47155]
- Modified [`SHOW TABLES`](../v20.2/show-tables.html) to return schema and table type. Furthermore, sequences will now appear in `SHOW TABLES`. Any user who relies on `SHOW TABLES` to return one column can use `SELECT table_name FROM [SHOW TABLES]` to get compatible behavior between previous [version](cluster-settings.html#setting-version)s of CockroachDB. [#46756][#46756]
...
- We previously allowed mixed type 20.1/19.2 upgrades to add precision types. However, the 19.2 nodes would disrespect the precision component and we documented that. To make this cleaner, we instead only allow precision to be specified in time-related types when the [version](cluster-settings.html#setting-version) upgrade is complete. [#47425][#47425]
...
- Added support for collection of multi-column statistics. By default, statistics are now collected on all prefixes of each index, in addition to 100 other individual columns. This feature can be disabled by setting the [cluster setting](../v20.2/cluster-settings.html) `[sql.stats.multi_column_collection.enabled](cluster-settings.html#setting-sql-stats-multi_column_collection-enabled)` to false. [#47729][#47729]
...
- The default [vectorized execution](../v20.2/vectorized-execution.html) mode is now "on", which means that all queries that can use vectorized execution will do so. To revert to previous behavior `SET CLUSTER SETTING [sql.defaults.vectorize](cluster-settings.html#setting-sql-defaults-vectorize)='201auto'`. [#48059][#48059]
...
- Fixed the behavior of `crdb_internal.zones` in mixed-[version](cluster-settings.html#setting-version) clusters. [#47173][#47173]
...
- Avoid a condition during rapid [version](cluster-settings.html#setting-version) upgrades where a node would refuse to start, claiming "a store is too old for running [version](cluster-settings.html#setting-version)". Before this bug fix, the workaround was to decommission the node, delete the store directory, and re-add it to the cluster as a new node. [#47358][#47358]
...
- The two settings `[kv.range_split.by_load_enabled](cluster-settings.html#setting-kv-range_split-by_load_enabled)` and `[kv.range_split.load_qps_threshold](cluster-settings.html#setting-kv-range_split-load_qps_threshold)` are documented but were incorrectly marked as non-public in the output of `SHOW CLUSTER SETTINGS`. This has been corrected. This bug was introduced in v20.1. [#48582][#48582]
...
- Re-allow `[diagnostics.forced_sql_stat_reset.interval](cluster-settings.html#setting-diagnostics-forced_sql_stat_reset-interval)`, `diagnostics.sql_stat_reset.interval` and `[external.graphite.interval](cluster-settings.html#setting-external-graphite-interval)` to set to their maximum values (24hr, 24hr and 15min respectively). This previously only excluded these values to be allowed. [#48758][#48758]
...
- The schema change workload is meant for testing the behavior of schema changes on clusters with nodes with minimum [version](cluster-settings.html#setting-version) 19.2. It will deadlock on earlier [version](cluster-settings.html#setting-version)s. [#49662][#49662]
