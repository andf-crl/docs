...
- The error payload returned to the client when a [`DATE`](../v21.1/date.html)/[`TIME`](../v21.1/time.html) con[version](cluster-settings.html#setting-version) fails now contains more details about the difference between the values provided and the values that are expected. [#63540][#63540] {% comment %}doc{% endcomment %}
