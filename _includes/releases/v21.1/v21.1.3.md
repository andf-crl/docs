...
- Syntax errors in the host-based authentication (HBA) configuration in cluster setting `[server.host_based_authentication.configuration](cluster-settings.html#setting-server-host_based_authentication-configuration)` are now logged on the [OPS channel](../v21.1/logging.html). [#66128][#66128] {% comment %}doc{% endcomment %}
...
- The `kafka_sink_config` object now supports a `[version](cluster-settings.html#setting-version)` configuration item to specify Kafka server [version](cluster-settings.html#setting-version)s. This is likely only necessary for old (Kafka 0.11/Confluent 3.3 or earlier) Kafka servers. Additionally, settings not specified in `kafka_sink_config` now retain their default values. [#66314][#66314] {% comment %}doc{% endcomment %}
