...
- Fixed a rare bug present in [v21.1 beta [version](cluster-settings.html#setting-version)s](index.html#testing-releases) that could cause rapid range splits and merges on a `GLOBAL` table to lead to a stuck leaseholder replica. The situation is no longer possible. [#64304][#64304]
- Fixed a bug in previous v21.1 beta [version](cluster-settings.html#setting-version)s that allowed the store rebalancer to spuriously down-replicate a range during normal operation. [#64303][#64303]
