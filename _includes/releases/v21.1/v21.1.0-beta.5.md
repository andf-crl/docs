...
- SQL executor data validation queries spawned by a schema change or a [`RESTORE`](../v21.1/restore.html) will now use [vectorized](../v21.1/vectorized-execution.html) query execution and [DistSQL](../v21.1/architecture/sql-layer.html#distsql) optimization if these are enabled in the [cluster settings](../v21.1/cluster-settings.html) `[sql.defaults.vectorize](cluster-settings.html#setting-sql-defaults-vectorize)` and `[sql.defaults.distsql](cluster-settings.html#setting-sql-defaults-distsql)`, respectively. This may improve the speed of these queries. [#64004][#64004] {% comment %}doc{% endcomment %}
