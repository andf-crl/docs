...
- Rows containing empty arrays in [`ARRAY`](../v21.1/array.html) columns are now contained in [GIN indexes](../v21.1/inverted-indexes.html). This change is backward-incompatible because prior [version](cluster-settings.html#setting-version)s of CockroachDB will not be able to recognize and decode keys for empty arrays. Note that rows containing `NULL` values in an indexed column will still not be included in GIN indexes. [#55970][#55970] {% comment %}doc{% endcomment %}
...
- Upgraded CockroachDB's [version](cluster-settings.html#setting-version) of Go to v1.15.4. [#56363][#56363] {% comment %}doc{% endcomment %}
...
- Added cluster settings to enable/ disable the `BACKUP` and `RESTORE` commands. Attempts to use these features while they are disabled returns an error indicating that the database administrator has disabled the feature. Example usage: `SET CLUSTER SETTING [feature.backup.enabled](cluster-settings.html#setting-feature-backup-enabled) = FALSE; SET CLUSTER SETTING [feature.backup.enabled](cluster-settings.html#setting-feature-backup-enabled) = TRUE; SET CLUSTER SETTING [feature.restore.enabled](cluster-settings.html#setting-feature-restore-enabled) = FALSE; SET CLUSTER SETTING [feature.restore.enabled](cluster-settings.html#setting-feature-restore-enabled) = TRUE;`. [#56533][#56533] {% comment %}doc{% endcomment %}
- Added cluster settings to enable/ disable the `IMPORT`, `EXPORT`, and changefeed  commands. Attempts to use these features while they are disabled returns an error indicating that the database administrator has disabled the feature. Example usage: `SET CLUSTER SETTING [feature.import.enabled](cluster-settings.html#setting-feature-import-enabled) = FALSE; SET CLUSTER SETTING [feature.import.enabled](cluster-settings.html#setting-feature-import-enabled) = TRUE; SET CLUSTER SETTING [feature.export.enabled](cluster-settings.html#setting-feature-export-enabled) = FALSE; SET CLUSTER SETTING [feature.export.enabled](cluster-settings.html#setting-feature-export-enabled) = TRUE; SET CLUSTER SETTING [feature.changefeed.enabled](cluster-settings.html#setting-feature-changefeed-enabled) = FALSE; SET CLUSTER SETTING [feature.changefeed.enabled](cluster-settings.html#setting-feature-changefeed-enabled) = TRUE;`. [#56872][#56872] {% comment %}doc{% endcomment %}
...
- Changed the underlying type for the [version](cluster-settings.html#setting-version) cluster setting. Previously, it was of an internal type representing "state machine", but now it's simply "[version](cluster-settings.html#setting-version)". This has no operational implications, but the `Type` column in `cockroach gen settings-list` now shows "[version](cluster-settings.html#setting-version)" instead of "custom validation". [#55994][#55994],[#56546][#56546] {% comment %}doc{% endcomment %}
...
- Adds a feature flag via cluster settings for the `CREATE STATISTICS` and `ANALYZE` feature. If a user attempts to use the command while disabled, an error indicating that the database administrator had disabled the feature is surfaced. Example usage: `SET CLUSTER SETTING [feature.stats.enabled](cluster-settings.html#setting-feature-stats-enabled) = FALSE; SET CLUSTER SETITNG [feature.stats.enabled](cluster-settings.html#setting-feature-stats-enabled) = TRUE;`. [#57076][#57076] {% comment %}doc{% endcomment %}
...
- Added the `uuid_generate_v4()` built-in function. It works exactly like `gen_random_uuid()` but was added for compatibility with Postgres [version](cluster-settings.html#setting-version)s older than PG13. [#57212][#57212]
...
- Added a new prometheus metric called `seconds_until_license_expiry` that reports the number of seconds until the [enterprise.license](cluster-settings.html#setting-enterprise-license) on the cluster expires, a negative number if the expiration is in the past, or `0` if there is no license. [#55565][#55565] {% comment %}doc{% endcomment %}
...
- Fixed a rare scenario in which a node would refuse to start after updating the binary. The log message would indicate: `store [...], last used with cockroach [version](cluster-settings.html#setting-version) [...], is too old for running [version](cluster-settings.html#setting-version) [...] (which requires data from [...] or later)`. [#55240][#55240]
...
- Fixed a bug which could cause `IMPORT`, `BACKUP`, or `RESTORE` to experience an error when they occur concurrently to when the cluster sets its [version](cluster-settings.html#setting-version) to upgraded. [#55524][#55524]
