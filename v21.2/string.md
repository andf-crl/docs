...
{% include_cached new-in.html [version](cluster-settings.html#setting-version)="v21.2" %} You can cast a `STRING` value of hexadecimal digits prefixed by `x` or `X` to a `BIT` value.
