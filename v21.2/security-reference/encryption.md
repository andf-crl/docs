...
- **Store key**: A cluster's _store key_ is a key encryption key (KEK) that CockroachDB uses to encrypt the cluster's data keys (see below).
  ...
  For CockroachDB Self-Hosted clusters, you provide the store key and give its location to CockroachDB when starting the cluster. The store key file must contain 32 bytes (the key ID) followed by the key (16, 24, or 32 bytes). The size of the key dictates the [version](cluster-settings.html#setting-version) of AES to use (AES-128, AES-192, or AES-256). For an example showing how to create a store key, see [Generating Key Files](/docs/{{site.[version](cluster-settings.html#setting-version)s["stable"]}}/encryption.html#generating-store-key-files).
