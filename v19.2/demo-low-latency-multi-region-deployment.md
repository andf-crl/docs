...
Some CockroachDB features used in this tutorial require an [enterprise.license](cluster-settings.html#setting-enterprise-license), so [request a 30-day trial license](https://www.cockroachlabs.com/get-cockroachdb/enterprise) before you get started.
...
Before you can run MovR against the cluster, you must create a `movr` database and enable an [enterprise.license](cluster-settings.html#setting-enterprise-license).
...
Be sure to use the exact [version](cluster-settings.html#setting-version) of MovR specified in the commands: `movr:19.09.2`. This tutorial relies on the SQL schema in this specific [version](cluster-settings.html#setting-version).
...
2. Use the [`SHOW CREATE TABLE`](show-create.html) statement to review the partition definition for one of the geo-specific tables:
    ...
    <span class="[version](cluster-settings.html#setting-version)-tag">New in v19.2:</span> The warning at the bottom tells you that partitions are not yet applied because corresponding replication zones still need to be created.
    {{site.data.alerts.end}}
...
3. At this point, you can use the [`SHOW CREATE TABLE`](show-create.html) statement to confirm that partitions are in effect:
    ...
    <span class="[version](cluster-settings.html#setting-version)-tag">New in v19.2:</span> The [`SHOW PARTITIONS`](show-partitions.html) statement is another way to confirm that partitions are in effect:
