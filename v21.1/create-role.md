...
{{site.data.alerts.callout_info}}
 <code>CREATE ROLE</code> is no longer an Enterprise feature and is now freely available in the core [version](cluster-settings.html#setting-version) of CockroachDB. Also, since the keywords `ROLE` and `USER` can now be used interchangeably in SQL statements for enhanced Postgres compatibility.
...
<section>{% include {{ page.[version](cluster-settings.html#setting-version).[version](cluster-settings.html#setting-version) }}/sql/generated/diagrams/create_role.html %}</section>
