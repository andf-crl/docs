...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v2.0-beta.20180305
sitemap: false
docs_area: releases
---
...
- Added a geographical map to the homepage of the Admin UI enterprise [version](cluster-settings.html#setting-version), showing the location of nodes and localities in the cluster. The map is annotated with several top-level metrics: storage capacity used, queries per second, and current CPU usage, as well as the liveness status of nodes in the cluster. [#22763][#22763]
...
- SQL execution logs enabled by the [cluster setting](../v2.0/cluster-settings.html) `[sql.trace.log_statement_execute](cluster-settings.html#setting-sql-trace-log_statement_execute)` now go to a separate log file. This is an experimental feature meant to aid troubleshooting CockroachDB. [#22534][#22534]
...
- Error messages displayed while connecting to a server with an incompatible [version](cluster-settings.html#setting-version) have been improved. [#22709][#22709]
...
- The con[version](cluster-settings.html#setting-version) from [`INTERVAL`](../v2.0/interval.html) to [`FLOAT`](../v2.0/float.html) now properly returns the number of seconds in the interval. [#22894][#22894]
...
- Fixed bugs when running DistSQL queries across mixed-[version](cluster-settings.html#setting-version) (1.1.x and 2.0-alpha) clusters. [#22897][#22897]
...
- Updated docs on [supporting castings for `ARRAY` values](../v2.0/array.html#supported-casting-con[version](cluster-settings.html#setting-version)new-in-v2-0). [#2549](https://github.com/cockroachdb/docs/pull/2549)
