...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v20.1.0-beta.2 since [version](cluster-settings.html#setting-version) v20.1.0-beta.1
docs_area: releases
---
...
In addition to various updates, enhancements, and bug fixes, this v20.1 beta release includes the ability to **log slow SQL queries**. By setting the `[sql.log.slow_query.latency_threshold](cluster-settings.html#setting-sql-log-slow_query-latency_threshold)` [cluster setting](../v20.1/cluster-settings.html), each node of your cluster will log queries that exceed the specified service latency to a new file called `cockroach-sql-slow.log`.
...
- Added a slow query log facility to CockroachDB, configurable by setting the `[sql.log.slow_query.latency_threshold](cluster-settings.html#setting-sql-log-slow_query-latency_threshold)` [cluster setting](../v20.1/cluster-settings.html). When used, each node of your cluster will record queries that exceed the specified service latency to a new file called `cockroach-sql-slow.log`. [#44816][#44816]
