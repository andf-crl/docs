...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) beta-20170413.
docs_area: releases 
---
...
- The implementation of indexing for [collated string columns](../v1.0/collate.html) has changed. Any secondary indexes containing collated strings must be [dropped](../v1.0/drop-index.html) before upgrading to this [version](cluster-settings.html#setting-version) and then [re-created](../v1.0/create-index.html) after upgrading. Note that this applies only to secondary indexes, not to primary indexes. [#14528](https://github.com/cockroachdb/cockroach/pull/14528)

- Any row with a `-0` [`FLOAT`](../v1.0/float.html) or [`DECIMAL`](../v1.0/decimal.html) value in a primary key or index must be [deleted](../v1.0/delete.html) before upgrading to this [version](cluster-settings.html#setting-version) and then [re-inserted](../v1.0/insert.html) after upgrading. Otherwise, the row containing `-0` will show up during table scans, but it will not be possible to delete it or select it as part of an index scan.
...
- Developers should use `make generate` instead of `go generate` to ensure the correct tool [version](cluster-settings.html#setting-version)s are used. [#14672](https://github.com/cockroachdb/cockroach/pull/14672)
