...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v1.1-alpha.20170608
sitemap: false
docs_area: releases 
---
...
- The new `[trace.debug.enable](cluster-settings.html#setting-trace-debug-enable)` cluster setting controls the collection of tracing data for the `/debug/requests` pages. The setting `trace.lightstep.token` can be used to send tracing data to LightStep. [#16265](https://github.com/cockroachdb/cockroach/pull/16265)
