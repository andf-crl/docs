...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v19.1.0.
sitemap: false
docs_area: releases
---
...
These features require an [[enterprise.license](cluster-settings.html#setting-enterprise-license)](../v19.1/enterprise-licensing.html). Register for a 30-day trial license [here](https://www.cockroachlabs.com/get-cockroachdb/enterprise).
...
[**Core Changefeeds**](../v19.1/changefeed-for.html) | CockroachDB now offers a non-enterprise [version](cluster-settings.html#setting-version) of change data capture, via the `EXPERIMENTAL CHANGEFEED FOR` statement, to consume table updates over a streaming Postgres connection.
[**Cost-Based Optimizer**](../v19.1/cost-based-optimizer.html) | The cost-based optimizer now supports almost all read-only queries (except window functions) and almost all mutations (e.g., `CREATE TABLE AS`, `INSERT`, `UPDATE`, `UPSERT`, `DELETE`). In addition, the cost-based optimizer now reorders up to 4 joins in a query to attempt to find the most performant ordering (via the new `reorder_joins_limit` [session variable](../v19.1/set-vars.html)) and takes advantage of [automatically generated table statistics](../v19.1/cost-based-optimizer.html#table-statistics) without impacting foreground traffic. Note that statistics are created by default on all indexed columns when a user upgrades to this [version](cluster-settings.html#setting-version). Finally, a [query plan cache](../v19.1/cost-based-optimizer.html#query-plan-cache) now saves a portion of the planning time for frequent queries used in the cost-based optimizer.
...
Before [upgrading to CockroachDB v19.1.0](../v19.1/upgrade-cockroach-[version](cluster-settings.html#setting-version).html), be sure to review the following backward-incompatible changes and adjust your application as necessary.
