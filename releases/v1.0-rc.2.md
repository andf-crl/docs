...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v1.0-rc.2
sitemap: false
docs_area: releases 
---
...
- New variables added to the [cluster-wide settings](../v1.0/cluster-settings.html): `server.declined_reservation_timeout`, `server.failed_reservation_timeout`, and `[server.time_until_store_dead](cluster-settings.html#setting-server-time_until_store_dead)`. [#15581](https://github.com/cockroachdb/cockroach/pull/15581)
...
- Queries using session-dependent variables (e.g., time con[version](cluster-settings.html#setting-version)s using the session's timezone) are now executed through the distributed SQL engine. [#15628](https://github.com/cockroachdb/cockroach/pull/15628)
