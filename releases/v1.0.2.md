...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v1.0.2
sitemap: false
docs_area: releases 
---
...
- `EXPLAIN (TRACE)` and the `[sql.trace.txn.enable_threshold](cluster-settings.html#setting-sql-trace-txn-enable_threshold)` cluster setting now work even when the `[trace.debug.enable](cluster-settings.html#setting-trace-debug-enable)` [cluster setting](../v1.0/cluster-settings.html) is false (the default). [#16217](https://github.com/cockroachdb/cockroach/pull/16217)
