...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v20.1.0-rc.1 since [version](cluster-settings.html#setting-version) v20.1.0-beta.4
docs_area: releases 
---
...
- Transactions reading a lot of data behave better when exceeding the memory limit set by the `[kv.transaction.max_refresh_spans_bytes](cluster-settings.html#setting-kv-transaction-max_refresh_spans_bytes)` [cluster setting](../v20.1/cluster-settings.html). Such transactions now attempt to resolve the conflicts they run into instead of being forced to always retry. Increasing `[kv.transaction.max_refresh_spans_bytes](cluster-settings.html#setting-kv-transaction-max_refresh_spans_bytes)` should no longer be necessary for most workloads. [#46803][#46803]
- Before [upgrading from v19.2 to v20.1](../v20.1/upgrade-cockroach-[version](cluster-settings.html#setting-version).html), it is best practice to make sure there are no schema changes in progress. However, if any are still running when the upgrade is started, note that they will stop making progress during the upgrade and will run to completion once the upgrade has been finalized. [#47073][#47073]
...
- Improved the error message for `ALTER COLUMN ... SET DATA TYPE` for data type con[version](cluster-settings.html#setting-version)s that involve overwriting existing values. [#47170][#47170]
...
- Fixed the behavior of `crdb_internal.zones` in mixed-[version](cluster-settings.html#setting-version) clusters. [#47236][#47236]
- Fixed reads from `system.namespace` and `crdb_internal.zones` on 19.2 nodes in a mixed-[version](cluster-settings.html#setting-version) cluster. [#47236][#47236]
