...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v19.2.0-alpha.20190805 since [version](cluster-settings.html#setting-version) v19.2.0-alpha.20190701
docs_area: releases 
---
...
- Added [cluster setting](../v19.2/set-cluster-setting.html) `[sql.stats.histogram_collection.enabled](cluster-settings.html#setting-sql-stats-histogram_collection-enabled)` to enable collection of histograms during statistics collection. [#38812][#38812]
...
- Ranges consisting of only one row (and historical [version](cluster-settings.html#setting-version)s of that row) are now correctly up-replicating. [#38529][#38529]
- Nodes that have been down now recover more quickly when they rejoin, assuming they weren't down for much more than the value of the `[server.time_until_store_dead](cluster-settings.html#setting-server-time_until_store_dead)` [cluster setting](../v19.2/set-cluster-setting.html) (which defaults to 5 minutes). [#38484][#38484]
...
- Building CockroachDB from source now requires node [version](cluster-settings.html#setting-version) 10. This was previously implicitly required. Now the requirement is explicit. [#38955][#38955]
