...
toc: true
summary: Additions and changes in CockroachDB [version](cluster-settings.html#setting-version) v19.1.0-beta.20190225 since [version](cluster-settings.html#setting-version) v2.2.0-alpha.20190211
sitemap: false
docs_area: releases 
---
...
Since our initial launch, Cockroach Labs has used semantic [version](cluster-settings.html#setting-version)ing in our release cycle guidelines. Two years, one major release, and n-patch fixes later, we're making the switch to Calendar Versioning. This means subscribers to our release notes will see quite the jump in today's [version](cluster-settings.html#setting-version) numbering, from last week's 2.1.5 to today's 19.1 beta. You can read more about the switch [here](https://www.cockroachlabs.com/blog/calendar-[version](cluster-settings.html#setting-version)ing).
...
- Fixed a panic when [`CREATE STATISTICS`](../v19.1/create-statistics.html) is run on clusters containing nodes with different [version](cluster-settings.html#setting-version)s of CockroachDB. [#34842][#34842]
